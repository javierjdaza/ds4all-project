...
def student_10():
    ...
    def make_prediction(df_input:pd.DataFrame):
        ...
        # if predict == 0:
        #     text_result = f'According to the features inputed, the student will have a Saber 10 Score below 340.  \n  The probability of the student for overcome the 340 score was: {predict_proba}%'
        #     st.error(text_result)
        # elif predict ==1:
        #     text_result = f'According to the features inputed, the student will have a Saber 10 Score above 340 with a probability of: {predict_proba}%'
    ...
    def create_column_feature_grades_10(name :str):
        min_value = 0 # int(df[name].min())
        max_value = 100 # int(df[name].max())
        name_display = name.capitalize()
        name_display = name_display.replace('_',' ').replace('notas 10','- 10 Grades').replace('nio','ño')
    ...
    # PSAT
    # =======================
    st.subheader('Psat Results - 10✍')
    p1,p2= st.columns(2)
    st.write('---')

    with p1:
        math_psat = create_column_feature_psat('math_psat')
    with p2:
        reading_and_writing_psat = create_column_feature_psat('reading_and_writing_psat')
    ...
    with g1:
        ciencias_notas_10 = create_column_feature_grades_10('ciencias_notas_10')
    with g2:
        ciencias_sociales_notas_10 = create_column_feature_grades_10('ciencias_sociales_notas_10')
    with g3:
        disciplina_notas_10 = create_column_feature_grades_10('disciplina_notas_10')
    with g4:
        espaniol_notas_10 = create_column_feature_grades_10('espaniol_notas_10')
    with g5:
        matematicas_notas_10 = create_column_feature_grades_10('matematicas_notas_10')
    with g6:
        geometria_notas_10 = create_column_feature_grades_10('geometria_notas_10')
    with g7:
        ingles_notas_10 = create_column_feature_grades_10('ingles_notas_10')
    with g8:
        social_studies_notas_10 = create_column_feature_grades_10('social_studies_notas_10')
    with g9:
        fisica_notas_10 = create_column_feature_grades_10('fisica_notas_10')
    ...
    dict_input = {
    'math_psat': math_psat,
    'reading_and_writing_psat': reading_and_writing_psat,
    'ciencias_notas_10': ciencias_notas_10,
    'ciencias_sociales_notas_10': ciencias_sociales_notas_10,
    'disciplina_notas_10': disciplina_notas_10,
    'espaniol_notas_10': espaniol_notas_10,
    'matematicas_notas_10': matematicas_notas_10,
    'geometria_notas_10': geometria_notas_10,
    'ingles_notas_10': ingles_notas_10,
    'matematicas_notas_10':matematicas_notas_10,
    'social_studies_notas_10':social_studies_notas_10,
    'fisica_notas_10':fisica_notas_10
    }
